create table IF NOT EXISTS win_fragment
(
...
;
create table IF NOT EXISTS win_match_award
(
  ...
  valid_status bit default 'true' not null comment '1有效，其他无效'
)
;

create table IF NOT EXISTS win_match_basic
(
  id bigint auto_increment
    primary key,
  match_owner int UNSIGNED null comment '比赛所有者',
  match_name varchar(100) not null comment '比赛名称',
  match_pic VARCHAR(50) not null,
  ...
  rule_config VARCHAR(500) null
)
;

create table IF NOT EXISTS win_match_join
(
  id bigint auto_increment
    primary key,
  ...
  join_date timestamp not null comment '加入比赛时间',
...
create index idx_win_match_join_accountId_status
  on win_match_join (account_id, is_deleted)
;

create index idx_win_match_join_matchId
  on win_match_join (match_id)
;

create table IF NOT EXISTS win_match_rule
(
  id bigint auto_increment
    primary key,
  ...
  valid_status bit default 'true' null comment '1有效，其他无效',
  bina int default '0' null comment '二进制值'
)
;

create index idx_win_match_rule_key
  on win_match_rule (`key`)
;

create index idx_win_match_rule_matchId
  on win_match_rule (matchId)
;

create table IF NOT EXISTS win_match_team_basic
(
  id bigint auto_increment comment '主键id'
    primary key,
  match_id int not null comment '段比赛ID',
  ...
  is_deleted tinyint unsigned default '0' null comment '1 表示删除，0 表示未删除',
  is_locked tinyint unsigned default '0' null comment '1表示已锁定',
  constraint uk_team_name
  unique (team_name)
)
;

create table IF NOT EXISTS win_match_team_join
(
  id bigint auto_increment comment '主键id'
    primary key,
  match_id int not null comment '段比赛ID',
  team_id int not null comment '战队ID',
  user_id varchar(20) not null comment '用户SSOID',
  account_id varchar(50) not null comment '用户账户id',
...
create index idx_win_match_team_join_teamId
  on win_match_team_join (team_id)
;

create index idx_win_match_team_join_userId
  on win_match_team_join (user_id)
;

create table IF NOT EXISTS win_match_team_member_stat_rank
(
  id bigint auto_increment
    primary key,
  match_id int not null comment '段比赛ID',
...
create index idx_win_match_team_member_stat_rank_team_id
  on win_match_team_member_stat_rank (team_id)
;

create table IF NOT EXISTS win_match_team_stat_rank
(
  id bigint auto_increment comment '主键id'
    primary key,
  match_id int not null comment '段比赛ID',
...
create table IF NOT EXISTS win_match_team_member_day_stat_rank
(
  match_id int not null comment '比赛ID',
  team_id bigint default '0' not null comment '团队Id',
  user_id varchar(20) not null comment '用户Id',
...
create table IF NOT EXISTS win_match_team_day_stat_rank
(
  match_id int not null comment '段比赛ID',
  team_id int not null comment '战队ID',
  member_num int default '0' null comment '战队人数',
...
create table IF NOT EXISTS win_match_user_stat_rank
(
  id bigint auto_increment comment '主键id'
    primary key,
  match_id int not null comment '比赛ID',
...
create index idx_win_match_user_stat_rank_stat_date
  on win_match_user_stat_rank (stat_date)
;

create table IF NOT EXISTS win_match_user_day_stat_rank
(
  match_id int not null comment '比赛ID',
  user_id int not null comment '用户id',
  account_id varchar(50) not null comment '账户总收益',
...
create table IF NOT EXISTS win_send_red_detail
(
  id bigint auto_increment comment '主键id'
    primary key,
  match_id int not null comment '段比赛ID',
...
create table IF NOT EXISTS win_stat_config
(
  id bigint auto_increment
    primary key,
  `key` varchar(100) null comment '配置key',
  value varchar(500) null comment 'key对应的value',
  valid bit default 'true' null comment '状态0：false 1:true'
)
;

create table IF NOT EXISTS win_trading
(
  id bigint auto_increment
    primary key,
  user_id varchar(20) not null comment '用户SSOID',
...
create index idx_win_trading_conclude_time_commission_type
  on win_trading (conclude_time, commission_type)
;

create index idx_win_trading_user_id
  on win_trading (user_id)
;

create table IF NOT EXISTS win_match_user
(
  id bigint auto_increment
    primary key,
...
create index idx_win_match_user_pid
  on win_match_user (passport_id)
;

create table IF NOT EXISTS win_match_user_account
(
  id bigint auto_increment
    primary key,
  user_id int not null comment '用户id',
...
create table IF NOT EXISTS win_match_user_account_stat_rank
(
...
create index idx_win_match_user_account_stat_rank_stat_date
  on win_match_user_account_stat_rank (stat_date)
;

create table IF NOT EXISTS win_match_user_account_day_stat_rank
(
